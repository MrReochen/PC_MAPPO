...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space, state_shape):
        ...
        state_shape = get_shape_from_obs_space(state_shape)
        ...
        self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *share_obs_shape),
                                  dtype=np.float32)
        self.state = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *state_shape),
                                  dtype=np.float32)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None, state=None):
        """
    ...
    def chooseinsert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
        ...
        self.state[self.step] = state.copy()
    ...
    def update_rewards(self, weights):
        self.rewards = self.rewards * weights
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if len(self.share_obs.shape) > 4:
            ...
            state = self.state[:-1].transpose(1, 2, 0, 3, 4, 5).reshape(-1, *self.state.shape[3:])
