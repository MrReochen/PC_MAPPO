...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.share_obs_type = args.share_obs_type
        ...
        for i in range(self.n_agents):
            ...
            self.state_space.append(self.get_state_dict_size())
    ...
    def reset(self):
        ...
        if self.share_obs_type == "full":
            global_state = [self.get_state_agent(agent_id) for agent_id in range(self.n_agents)]
        elif self.share_obs_type == "half":
            global_state = [self.get_state(agent_id) for agent_id in range(self.n_agents)]
        elif self.share_obs_type == "none":
            global_state = [np.concatenate((self.get_obs_agent(agent_id), self.get_state_dict()), axis=-1) for agent_id in range(self.n_agents)]
        ...
        return local_obs, global_state, [self.get_state_dict() for _ in range(self.n_agents)], available_actions
    ...
    def step(self, actions):
        ...
        try:
            ...
            return local_obs, global_state, [self.get_state_dict() for _ in range(self.n_agents)], [[0]]*self.n_agents, dones, infos, available_actions
        ...
        return local_obs, global_state, [self.get_state_dict() for _ in range(self.n_agents)], rewards, dones, infos, available_actions
    ...
    def get_state_size(self):
        ...
        if self.share_obs_type == "full":
            ...
            local_obs_size = self.get_obs_size()
            state_dict_size = self.get_state_dict_size()
            local_obs_size[0] += state_dict_size[0]
